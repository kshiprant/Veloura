  ...
  return (
    <AppShell>
      <div className="stack gap16">
        ...
        {!loading && !current ? <div className="soft-card">No more profiles for now. Come back later.</div> : null}
        {current ? (
          <div className="profile-card soft-card">
            <img src={current.photos?.[0] || 'https://placehold.co/800x1000?text=Veloura'} alt={current.firstName} className="hero-image" />
            <div className="profile-body">
              ...
              <div className="chip-grid">
                {(current.interests || []).map((item) => <span key={item} className="chip-tag">{item}</span>)}
              </div>
            ...
            <div className="footer-actions no-top">
              <button className="button outline half" onClick={() => setProfiles((prev) => prev.slice(1))}>Pass</button>
              <button className="button primary half" onClick={() => like(current._id)}>Like</button>
            </div>
          </div>
        ) : null}
      </div>
