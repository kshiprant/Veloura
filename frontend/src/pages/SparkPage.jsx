...
export default function SparkPage() {
  ...
  const loadSession = async (sessionId) => {
    try {
      ...
      setSession({ ...data.session, currentUserId: data.currentUserId });
  ...
  const startConversation = async () => {
    try {
      ...
      setRevealAvailable(Boolean(data.revealAvailable));
      setSessionsLeft(
        typeof data.sessionsLeft === 'number' ? data.sessionsLeft : 7
      );
      setStatus('connected');
          ...
          {status === 'connected' && (
            <div className="spark-chat-card">
              ...
              <div className="spark-messages-list">
                {messages.map((msg) => {
                  const mine =
                    msg?.sender?._id === session?.currentUserId ||
                    msg?.sender === session?.currentUserId;
                  ...
                  if (msg.type === 'reveal_request') {
                    const isForMe =
                      msg?.targetUser === session?.currentUserId ||
                      msg?.targetUser?._id === session?.currentUserId;
                    ...
                    return (
                      <div key={msg._id} className="spark-reveal-card">
                        ...
                        {isForMe ? (
                          ...
                          <p className="small-text muted">Waiting for their response.</p>
                        )}
                      </div>
                    );
                  }
